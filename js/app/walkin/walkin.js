...
export class Walkin extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('http://icore.repairmonk.com/website/walkin_location/', JSON.stringify(axiosConfig))
      .then(response => {
        console.log(response);
  ...
  handleAddJob(e) {
    e.preventDefault();
    axios.post('http://icore.repairmonk.com/website/createjob/', {customer_id: cookies.get('userId'), location_code: this.state.deliveryCentre, address_id: 14813, device_id: cookies.get('device-model-id'), problems: cookies.get('device-issues'), colour: cookies.get('device-colour'), type: 'Service Center'}, JSON.stringify(axiosConfig))
      .then(response => {
        console.log(response);
